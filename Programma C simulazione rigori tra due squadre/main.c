...
int main()
{
    int nx, ny, nz, nc, i, j, a=1, c=-4, accumulatore=0, accumulatore1=0;
    ...
    while(a<=5 || c<=5){
        ...
        if(a<=5 && c<=5)
            printf("\n%d Rigore squadra Milan\n", a);
        else if(c>=0)
            printf("\n%d Rigore squadra Inter\n", c);
        ...
        if(a<=6 && c<=1){                               /*Accumulatore che tiene conto dei goal fatti dal Milan*/
            if(x[nx][ny] != x[nz][nc])
            accumulatore++;}


        if(c>1){                                        /*Accumulatore che tiene conto dei goal fatti dal Inter*/
            if(x[nx][ny] != x[nz][nc])
            accumulatore1++;}
    ...
    printf("\n\n");
    if(accumulatore > accumulatore1)
        printf("VITTORIA Milan");
    else if(accumulatore < accumulatore1)
        printf("VITTORIA Inter");
    else if(accumulatore == accumulatore1)
        printf("PARITA'");

    printf("\n\nACCUMULATORE: %d\nACCUMULATORE1: %d\n", accumulatore, accumulatore1);

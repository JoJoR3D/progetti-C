...
/*Pongo riga e colonna == 0 perchè, per entrare nel while di sotto, mi serve che siano diversi da tot (posso anche non inizzzializzarle a 0 utilizzando il ciclo
  do while al posto del ciclo while. Cosa che ho fatto. il do while mi consente di entrare nel ciclo almeno una volta, eseguirlo e poi controllare la condizione
  del while).
  Se al primo ciclo ho inserito, tremite scanf, ad esempio:
  m=4 e n=4, ora tot= 4*4= 16.
  Entrato nel ciclo do while genero casualmente dei valori (da 1 a un massimo di 99) per colonna, invece per riga (da 2 a un massimo di 99. Ma può generare anche
  il valore 1: 2+99%100= 101%100= 1.
  Infatti se viene generato il valore 1, per la riga, viene generato nuovamente un altro valore. Con 1 sola riga l'algoritmo non ordina in maniera corretta i valori
  del array 'a'.
  Il valore 1 come riga non mi serve!). Moltiplico riga*colonna il cui deve essere uguale a tot.

  Ad esempio se riga= 3 e colonna= 4; 3*4 != tot=16 ? SI! allora rimango nel ciclo do while. Uscirò dal do while solo se il valore generato per riga, moltiplicato
  per il valore generato per colonna sarà uguale a tot.
  Ad esempio: se riga= 2 e colonna= 8; 2*8 != tot=16 ? NO! perché sono uguali, quindi esco dal ciclo while.

  Entrato nel ciclo do while viene generato un valore pseudocasuale per riga e per colonna.
  Se il valore generato per riga è ==1 entro nel ciclo while sottostante e genero un altro valore pseudocasuale per riga. Uscirò da questo while solo quando il
  valore generato per riga sarà diverso da 1. Questo perchè con 1 sola riga l'algoritmo non ordina in maniera corretta i valori del array 'a'.
  Il valore 1 come riga non mi serve!

  Altra condizione che mi fa entrare in questo ciclo while è colonna > n. L'ALGORITMO, PER FUNZIONARE CORRETTAMENTE, AD OGNI CILO IL VALORE DI COLONNA DEVE
  DIMINUIRE, quindi mi assicuro che venga generato nel ciclo in corso un valore più piccolo rispetto ai cicli precedenti.
  Colonna contiene il nuovo valore generato a questo ciclo, mentre 'n' contiene il valore di colonna generato al precedente ciclo.

  Terminato questo ciclo while, sempre nel ciclo do while c'è un ciclo for che verifica che il valore generato per colonna non sia già stato generato nei cicli precedenti.
  Ricordo che L'ALGORITMO, PER FUNZIONARE CORRETTAMENTE, AD OGNI CILO IL VALORE DI COLONNA DEVE DIMINUIRE. Come verifico ciò? L'array vet contiene i valori di
  colonna('n') utilizzati in ogni ciclo.

  Infatti questo ciclo for poi salva il valore di colonna nel array vet nelle posizioni dove c'e 0 se anche riga*colonna è ==tot.
  Senza riga*colonna ==tot, ad ogni ciclo do while che si avrà, il valore di colonna sarà salvato nel array,
  invece così sarà salvato solo il valore di colonna corretto e che di conseguenza farà terminare il do while.

  Se moltiplicando i valori generati per riga e colonna, il risultato sarà uguale a tot, allora il ciclo do while terminerà.

  Poi passo a 'm' il valore di riga, a 'n' il valore di colonna e a 'n_col' il valore di colonna.

  RICORDO che: Il valore di riga moltiplicato per il valore di colonna deve essere == a tot per il discorso fatto prima:
  Se al Primo ciclo ho inserito, ad esempio, n=4 e m=4, 4*4= 16 valori, quindi nel secondo ciclo devo inserire
  dei valori di 'n' e 'm' che mi permettano di contenere 16 valori, ad esempio: n=2, m=8, 2*8= 16 valori.
  E questo vale anche per i successivi cicli finchè non inserirò che 'n' (num.colonne) è == 1.*/

...
int main()
{
    ...
    while(1){                       /*while(1) indice che il ciclo si ripeterà al infinito a meno che non venga interrotto con il comando break.*/
        ...
        if(a[i] > a[i+1]){          /*Al primo ciclo: Se il primo elemento del array è > del secondo elemento questo viene scambiato tramite lo swap.*/
            swap(&a[i],&a[i+1]);
            s=i;                    /*Viene salvato l'indice del ultimo elemento scambiato, e questa sarà la nuova dimensione del array al prossimo ciclo.
                                      Questo perchè dopo questo elemento, gli altri elementi sono sicuramente già ordinati. Quindi non c'è bisogno di confrontarli
                                      per i prossimi scambi.*/
            printf("%d\n",s);
        ...
        if(i+1>=n && cont != 0){    /*Se i+1 è >= n vuol dire che sono arrivato alla fine dell'array e quindi devo ripristinare gli indici per ricominciare dal inizio*/
            ...
            n=s+1;                  /*Dopo il primo ciclo sicuramente al ultima posizione del array ci sarà l'elemento più grande, quindi al ciclo successivo sarà inutile
                                      ...
                                      Quindi ad ogni iterazione viene ridotta la dimensione del array.
                                      La nuova dimensione dipende dal punto in cui si è verificato l'ultimo scambio. Quella sarà la nuova dimensione del array.
                                      In questo modo si evita di riconfrontare gli elementi già ordinati.*/
